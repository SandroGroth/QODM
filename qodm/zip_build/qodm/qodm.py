...
class QODM:
    ...
    def get_orthophoto_build_overview(self):
        return self.dlg.ch_orthophoto_build_overview.isChecked()

    def get_orthophoto_use_3d_mesh(self):
        return self.dlg.ch_orthophoto_use_3d_mesh.isChecked()
    ...
    def set_variables(self):
        ...
        self.orthophoto_resolution = self.get_orthophoto_resolution()
    ...
    def build_command(self):
        ...
        if self.out_crs_proj4:
            options = options + ' --proj ' + '"' + self.out_crs_proj4 + '"'

        # add orthophoto options
        if self.orthophoto_resolution not in [None, 5]:
            options = options + ' --orthophoto-resolution ' + str(self.orthophoto_resolution)
        if self.orthophoto_compression not in [None, 'DEFLATE']:
            options = options + ' --orthophoto-compression' + self.orthophoto_compression
        if self.orthophoto_bigtiff not in [None, 'IF_SAFER']:
            options = options + ' --orthophoto-bigtiff' + self.orthophoto_bigtiff
        if self.orthophoto_fast:
            options = options + ' --fast-orthophoto'
        if self.orthophoto_not_tiled:
            options = options + ' --orthophoto-no-tiled'
        if self.orthophoto_cutline:
            options = options + ' --orthophoto-cutline'
        if self.orthophoto_build_overview:
            options = options + ' --build-overviews'
        if self.orthophoto_use_3d_mesh:
            options = options + ' --use-3dmesh'

        # add DEM options
        if self.dem_resolution not in [None, 5]:
            options = options + ' --dem-resolution ' + str(self.dem_resolution)
        if self.dem_gapfill_steps not in [None, 3]:
            options = options + ' --dem-gapfill-steps ' + str(self.dem_gapfill_steps)
        if self.dem_decimation not in [None, 1]:
            options = options + ' --dem-decimation ' + str(self.dem_decimation)
        if self.dem_euclidian_map:
            options = options + ' --dem-euclidean-map'
    ...
    def execute(self):
        ...
        # DEBUG os.system('docker run -ti --rm -v C:\\Projects\\HIWI\\UAV_Preprocessing\\Data\\LP_23_boreal_highway184:/datasets/code opendronemap/odm --project-path /datasets')#, stdout=subprocess.PIPE, shell=True, text=True)
